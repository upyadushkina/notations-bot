...
async function tryTelegramFallback(token, db, env, user, chatId) {
  await sendMessage(token, chatId, escapeHtml(t("regCheckingTelegram")));
  const p = await findByTelegramUsername(db, user.username);
  if (p) {
    await offerParticipantChoices(token, chatId, [p], "regMatchFound");
    // stash as single suggestion confirm via buttons
    return;
  }
...
async function handleCallback(token, db, env, cq) {
  ...
  if (data.startsWith("reg:")) {
    await answerCallbackQuery(token, cq.id);
    if (data === "reg:none") {
      await tryTelegramFallback(token, db, env, user, chatId);
      return;
    }
    ...
    if (m) {
      ...
      const kb = [
        ...
        [{ text: t("btnNotMe"), callback_data: "reg:none" }],
      ];
...
async function handleMessage(token, db, env, msg) {
  ...
  if (isStartCommand(text)) {
    await clearUserState(db, user.id);
    await navResetMenu(db, user.id);
    if (user.reg_status === "registered" || (admin && user.participant_id)) {
      const menu = await renderMainMenu(admin);
      await sendMessage(token, chatId, escapeHtml(t("welcome")), menu.keyboard);
      return;
    }
    // Try auto-match by telegram first for convenience, else ask name
    const byTg = await findByTelegramUsername(db, from.username);
    if (byTg && !byTg.linked_telegram_id) {
      const kb = [
        [{ text: t("btnConfirmItsMe"), callback_data: `reg:yes:${byTg.id}` }],
        [{ text: t("btnNotMe"), callback_data: "reg:notme" }],
      ];
      await setUserState(db, user.id, "awaiting_name", {});
      await sendMessage(
        token,
        chatId,
        `${escapeHtml(t("welcome"))}\n\n${escapeHtml(t("regMatchFound", { name: displayName(byTg) }))}\n${escapeHtml(t("regMatchConfirm"))}`,
        kb
      );
      return;
    }
  ...
  if (user.state === "awaiting_name" || (user.reg_status !== "registered" && text && !text.startsWith("/"))) {
    if (user.reg_status === "registered") {
      ...
      const people = await allParticipants(db);
      const matches = rankParticipants(people, text, 5);
      if (!matches.length) {
        await tryTelegramFallback(token, db, env, user, chatId);
        return;
      }
